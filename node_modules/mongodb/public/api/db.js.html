...
<div id="wrap" class="clearfix" style="width:100%;">
    <table style="height:100%;width:100%">
        <tr>
            <td valign='top'>
                <div class="main">
                    <h1 class="page-title" data-filename="db.js.html">Source: lib/db.js</h1>
            ...
            <pre class="prettyprint source linenums"><code>var EventEmitter = require('events').EventEmitter
                ...
                * @param {string} [options.authSource=null] If the database authentication is dependent on another
                databaseName.
                ...
                * @param {boolean} [options.promoteLongs=true] Promotes Long values to number if they fit inside the 53
                bits resolution.
                * @param {number} [options.bufferMaxEntries=-1] Sets a cap on how many operations the driver will buffer
                up before giving up on getting a working connection, default is -1 which is unlimited.
                ...
                * @param {object} [options.pkFactory=null] A primary key factory object for generation of custom _id
                keys.
                ...
                */
                ...
                /**
                * Fetch a specific collection (containing the actual collection information). If the application does
                not use strict mode you can
                ...
                */
                ...
                throw utils.toError(f("A callback is required in strict mode. While getting collection %s.",
                collectionName));
                }
                ...
                if(collections.length == 0) return handleCallback(callback, toError(f("Collection %s does not exist.
                Currently in strict mode.", name)), null);

                try {
                return handleCallback(callback, null, new Collection(self, topology, databaseName, name, pkFactory,
                options));
                } catch(err) {
                ...
                * @param {boolean} [options.autoIndexId=true] Create an index on the _id field of the document, True by
                default on MongoDB 2.2 or higher off for version &lt; 2.2.
                ...
                */
                ...
                return handleCallback(callback, new MongoError(f("Collection %s already exists. Currently in strict
                mode.", name)), null);
                } else if (collections.length > 0) {
                try { return handleCallback(callback, null, new Collection(self, topology, databaseName, name,
                pkFactory, options)); }
                ...
                */
                ...
                } else if(parameters != null &amp;&amp; Array.isArray(parameters) &amp;&amp; typeof parameters !==
                'function') {
                finalParameters = parameters;
                }
                ...
                */
                ...
                /**
                * Drop a collection from the database, removing it permanently. New accesses will create a new
                collection.
                *
                * @method
                ...
                */
                ...
                return new Collection(self, topology, databaseName, d.name.replace(databaseName + ".", ''), pkFactory,
                options);
                }));
                });
                };
                ...
                */
                ...
                // Insert document
                topology.insert(f("%s.%s", databaseName, Db.SYSTEM_INDEX_COLLECTION), doc, finalOptions, function(err,
                result) {
                if(callback == null) return;
                if(err) return handleCallback(callback, err);
                ...
                */
                ...
                throw utils.toError("The digestPassword option is not supported via add_user. Please use
                db.command('createUser', ...) instead for this option.");
                }
                ...
                if((self.databaseName.toLowerCase() == 'admin' || options.dbName == 'admin') &amp;&amp;
                ...
                if(err &amp;&amp; err.ok == 0 &amp;&amp; err.code == undefined) return handleCallback(callback, {code:
                -5000}, null);
                if(err) return handleCallback(callback, err, null);
                ...
                * @param {number} [options.wtimeout=null] The write concern timeout.
                * @param {boolean} [options.j=false] Specify a journal write concern.
                * @param {object} [options.customData=null] Custom data associated with the user (only Mongodb 2.6 or
                higher)
                * @param {object[]} [options.roles=null] Roles associated with the created user (only Mongodb 2.6 or
                higher)
                * @param {Db~resultCallback} callback The command result callback
                * @return {null}
                */
                ...
                collection.update({user: username},{$set: {user: username, pwd: userPassword}}, finalOptions,
                function(err, results, full) {
                if(count == 0 &amp;&amp; err) return handleCallback(callback, null, [{user:username,
                pwd:userPassword}]);
                if(err) return handleCallback(callback, err, null)
                ...
                // Execute the command
                self.command(command, commandOptions, function(err, result) {
                if(err &amp;&amp; !err.ok &amp;&amp; err.code == undefined) return handleCallback(callback, {code:
                -5000});
                if(err) return handleCallback(callback, err, null);
                handleCallback(callback, null, result.ok ? true : false);
                })
                ...
                * @param {string} [password] The password.
                * @param {object} [options=null] Optional settings.
                * @param {string} [options.authMechanism=MONGODB-CR] The authentication mechanism to use, GSSAPI,
                MONGODB-CR, MONGODB-X509, PLAIN
                * @param {Db~resultCallback} callback The command result callback
                * @return {null}
                */
                ...
                return handleCallback(callback, new MongoError("only GSSAPI, PLAIN, MONGODB-X509, SCRAM-SHA-1 or
                MONGODB-CR is supported by authMechanism"));
                }
                ...
                handleCallback(callback, new MongoError(f("authentication mechanism %s not supported",
                options.authMechanism), false));
                }
                };
                ...
                */
                ...
                if(databaseName.indexOf(invalidChars[i]) != -1) throw new MongoError("database names cannot contain the
                character '" + invalidChars[i] + "'");
                }
                }
