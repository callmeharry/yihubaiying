    ...
    <table style="height:100%;width:100%">
        <tr>
            ...
            <pre class="prettyprint source linenums"><code>/**
                ...
                * @param {string} [options.root=null] Root collection to use. Defaults to
                **{GridStore.DEFAULT_ROOT_COLLECTION}**.
                * @param {string} [options.content_type=null] MIME type of the file. Defaults to
                **{GridStore.DEFAULT_CONTENT_TYPE}**.
                * @param {number} [options.chunk_size=261120] Size for the chunk. Defaults to
                ...
                */
                ...
                this.internalChunkSize = this.options['chunkSize'] == null ? Chunk.DEFAULT_CHUNK_SIZE :
                ...
                *
                * @method
                * @param {Gridstore~resultCallback} callback this gets called after this method is executed. Passes null
                to the first parameter and the character read to the second or null to the second if the read/write head
                ...
                * @param {Gridstore~resultCallback} callback this will be called after executing this method. The first
                parameter will contain null and the second one will contain a reference to this object.
                * @return {null}
                */
                ...
                *
                * @method
                * @param {boolean} autoclose if true current GridStore will be closed when EOF and 'close' event will be
                fired
                ...
                *
                * @method
                * @param {number} [length] the number of characters to read. Reads all the characters from the
                read/write head to the EOF if not specified.
                * @param {(string|Buffer)} [buffer] a string to hold temporary data. This is used for storing the string
                data read so far when recursively calling this method.
                ...
                */
                ...
                if(finalLength == 0 &amp;&amp; finalBuffer.length == 0) return callback(new MongoError("File does not
                exist"), null);
                // Else return data
                ...
                */
                ...
                self.internalChunkSize = self.internalChunkSize == null ? Chunk.DEFAULT_CHUNK_SIZE :
                self.internalChunkSize;
                self.length = 0;
                } else {
                self.length = 0;
                var txtId = self.fileId instanceof ObjectID ? self.fileId.toHexString() : self.fileId;
                return error(new MongoError(f("file with id %s not opened for writing", (self.referenceBy ==
                REFERENCE_BY_ID ? txtId : self.filename))), self);
                }
                ...
                if(err) return error(err);
                self.currentChunk = new Chunk(self, {'n':0}, self.writeConcern);
                self.contentType = self.options['content_type'] == null ? self.contentType :
                self.options['content_type'];
                self.internalChunkSize = self.options['chunk_size'] == null ? self.internalChunkSize :
                ...
                */
                ...
                if(self.mode != "w") {
                callback(new MongoError(f("file with id %s not opened for writing", (self.referenceBy == REFERENCE_BY_ID
                ? self.referenceBy : self.filename))), null);
                } else {
                ...
                * @param {string} [rootCollection] The root collection that holds the files and chunks collection.
                ...
                */
                ...
                // Build query
                var query = (typeof fileIdObject == 'string' || Object.prototype.toString.call(fileIdObject) == '[object
                RegExp]' )
                ...
                */
                ...
                if(offset &amp;&amp; offset >= gridStore.length) return callback("offset larger than size of file",
                null);
                if(length &amp;&amp; length > gridStore.length) return callback("length is larger than the size of the
                file", null);
                if(offset &amp;&amp; length &amp;&amp; (offset + length) > gridStore.length) return callback("offset and
                length is larger than the size of the file", null);
                ...
                */
                ...
                if(options.w != null || typeof options.j == 'boolean' || typeof options.journal == 'boolean' || typeof
                ...
                } else if(self.options.w != null || typeof self.options.j == 'boolean' || typeof self.options.journal ==
                'boolean' || typeof self.options.fsync == 'boolean') {
                finalOptions = _setWriteConcernHash(self.options);
                } else if(self.safe.w != null || typeof self.safe.j == 'boolean' || typeof self.safe.journal ==
                'boolean' || typeof self.safe.fsync == 'boolean') {
                ...
                if(finalOptions.w &lt; 1
                &amp;&amp; (finalOptions.journal == true || finalOptions.j == true || finalOptions.fsync == true)) throw
                new MongoError("No acknowledgement using w &lt; 1 cannot be combined with journal:true or fsync:true");
                ...
                */
                ...
                if(endCalled) return self.emit('error', new MongoError('attempting to write to stream after end
                called'))
                ...
                */
                ...
                /**
                * Call this function to cause the stream to return strings of the specified encoding instead of Buffer
                objects.
                ...
                */
                ...
                /**
                * This method will cause a stream in flowing-mode to stop emitting data events. Any data that becomes
                ...
                */
                ...
                /**
                * This is useful in certain cases where a stream is being consumed by a parser, which needs to
                "un-consume" some data that it has optimistically pulled out of the source, so that the stream can be
                ...
                */
                ...
                /**
                * Call this method when no more data will be written to the stream. If supplied, the callback is
                attached as a listener on the finish event.
