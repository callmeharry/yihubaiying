            ...
            <ul class="sidebar-menu">
                ...
                <li class="sub-menu active">
                    ...
                    <ul class="sub open">
                        ...
                        <li class="active"><a href="http://mongodb.github.io/tutorials/urls/"> Connection URI </a></li>
                ...
                <li><a href="https://github.com/mongodb/node-mongodb-native/docs/content/tutorials/urls.md"
...
<pre><code>mongodb://[username:password@]host1[:port1][,host2[:port2],...[,hostN[:portN]]][/[database][?options]]
                            ...
                            <p>The URL format is unified across official drivers from Mongodb with some options not
                                supported on some drivers due to implementation differences. The ones not supported by
                                the Node.js driver are left out for simplicities sake.</p>
                            ...
                            <ul>
                                ...
                                <li><code>?options</code> are connection options. Note that if database is absent there
                                    is still a / required between the last host and the ? introducing the options.
                                    Options are name=value pairs and the pairs are separated by &ldquo;&amp;&ldquo;. For
                                    any unrecognized or unsupported option, a driver should log a warning and continue
                                    processing. A driver should not support any options that are not explicitly defined
                                    in this specification. This is in order to reduce the likelihood that different
                                    drivers will support overlapping that differ in small but incompatible ways (like
                                    different name, different values, or different default value).
                                </li>
                            </ul>
                            ...
                            <ul>
                                <li><code>replicaSet=name</code>
                                    ...
                                    <ul>
                                        <li>The driver verifies that the name of the replica set it connects to matches
                                            this name. Implies that the hosts given are a seed list, and the driver will
                            ...
                            <ul>
                                <li><p><code>ssl=true|false|prefer</code></p>
                                    ...
                                    <ul>
                                        ...
                                        <li>prefer: the driver tries to initiate each connection with SSL, and falls
                                            back to without SSL if it fails.
                                        </li>
                                ...
                                <li><p><code>connectTimeoutMS=ms</code></p>
                                    ...
                                    <ul>
                                        ...
                                        <li>Current driver behavior already differs on this, so the default must be left
                                            to each driver. For new implementations, the default should be to never
                                            timeout.
                                        </li>
                                    </ul>
                                </li>
                            ...
                            <p>More detailed information about write concerns can be found at <a
                                    href="http://www.mongodb.org/display/DOCS/getLastError+Command">http://www.mongodb.org/display/DOCS/getLastError+Command</a>
                            </p>
                            ...
                            <ul>
                                <li><p><code>w=wValue</code></p>
                                    ...
                                    <ul>
                                        ...
                                        <li>wValue is typically a number, but can be any string in order to allow for
                                            specifications like &ldquo;majority&rdquo;</li>
                                        <li>Default value is 1.
                                            ...
                                            <ul>
                                                ...
                                                <li>wValue == &lsquo;majority&rsquo; perform a write acknowledgement
                                                    across the majority of servers in the replicaset
                                                </li>
                                                <li>wValue == &lsquo;tag name&rsquo; perform a write acknowledgement
                                                    against the replicaset tag name
                                                </li>
                                            </ul>
                                        </li>
                                    </ul>
                                ...
                                <li><p><code>fsync=true|false</code></p>
                                    ...
                                    <ul>
                                        ...
                                        <li>Default value is false</li>
                                        <li>If conflicting values for fireAndForget, and any write concern are passed
                                            the driver should raise an exception about the conflict.
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                            ...
                            <ul>
                                <li><p><code>slaveOk=true|false:</code> Whether a driver connected to a replica set will
                                    send reads to slaves/secondaries.</p>
                                ...
                                <li><p><code>readPreference=enum:</code> The read preference for this connection. If
                                    set, it overrides any slaveOk value.</p>
                                ...
                                <li><p><code>readPreferenceTags=string.</code> A representation of a tag set as a
                                    comma-separated list of colon-separated key-value pairs, e.g.
                                    <code>dc:ny,rack:1</code>. Spaces should be stripped from beginning and end of all
                                    keys and values. To specify a list of tag sets, using multiple readPreferenceTags,
                                    e.g. <code>readPreferenceTags=dc:ny,rack:1&amp;readPreferenceTags=dc:ny&amp;readPreferenceTags=</code>
                                </p>

                                    <ul>
                                        <li>Note the empty value, it provides for fallback to any other secondary server
                ...
                <div class="col-md-1">
                    ...
                    <a class="navigation next" href="/tutorials/crud_operations">
