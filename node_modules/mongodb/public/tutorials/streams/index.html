            ...
            <ul class="sidebar-menu">
                ...
                <li class="sub-menu active">
                    ...
                    <ul class="sub open">
                        ...
                        <li class="active"><a href="http://mongodb.github.io/tutorials/streams/"> Streams </a></li>
                ...
                <li><a href="https://github.com/mongodb/node-mongodb-native/docs/content/tutorials/streams.md"
                ...
                <div class="col-md-10">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <p>The MongoDB driver has extensive Stream support for cursors as well as for GridFS. In
                                essence the following aspects of the driver supports Node 0.10.x or higher style
                                streams.</p>

                            <ul>
                                <li><code>find</code> The cursor returned from the <em>find</em> method is a <em>Readable</em>
                                    stream.
                                </li>
                                <li><code>aggregate</code> The cursor returned from the <em>aggregate</em> is a <em>Readable</em>
                                    stream.
                                </li>
                                <li><code>parallelCollectionScan</code> Returns an array of one or more cursors that all
                                    are <em>Readable</em> streams.
                                </li>
                                <li><code>GridStore.prototype.stream</code> Returns a stream that implements
                                    <em>Duplex</em> allowing for writing data in <em>w</em> mode and reading data in
                                    <em>r</em> mode.
                                </li>
                            </ul>
                            ...
                            <p>Let&rsquo;s examine a simple query using <em>find</em> and how to use it as a node.js
                                stream.</p>
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    db.close();
                            ...
                            <p>A very simple and straight forward stream of documents. For each document the cursor will
                                emit the <em>data</em> event and when the cursor has been exhausted it will issue the
                                <em>end</em> event. To transform the data you can pipe the data from this stream into
                                another stream. We will not show that here but there are a wide variety of stream based
                                libraries available on <a href="http://npmjs.org">NPM</a>.</p>

                            <p>The stream is in object mode meaning it will emit the actual document instances. If you
                                for some reason need this to be a different output you can use the <code>stream</code>
                                function on the cursor to supply a transformation method that will be called for each
                                document before it&rsquo;s emitted. Let&rsquo;s take a look at a simple example that
                                uses <em>JSON.stringify</em> to convert each document to it&rsquo;s JSON string
                                representation.</p>
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    db.close();
                            ...
                            <p>That wraps up the behaviors of the <em>Readable</em> stream for the <em>find</em> method.
                                Next let&rsquo;s look at the aggregate command.</p>

                            <h2 id="toc_2">Aggregation Cursor as a Stream</h2>

                            <p>The aggregation cursor behaves very much like the <em>find</em> cursor. It&rsquo;s main
                                difference is that it does not support a <em>transform</em> method. Let&rsquo;s have a
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    db.close();
                            ...
                            <p>As one can see the cursor behaves in the exact same way as the cursor that is returned
                                when invoking the <em>find</em> method. Let&rsquo;s have a look at the <em>parallelCollectionScan</em>
                            ...
                            <p>The <em>parallelCollectionScan</em> method is a specialized method that allows for
                                parallel reading of a collection using multiple cursors. This method is only available
                                when connecting to a single server or replicaset topology. Let&rsquo;s look at an
                                example.</p>
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    db.close();
                            ...
                            <p>In this example we use each cursor as a stream and when all cursors have emitted the <em>end</em>
                                event we check that the number of inserted documents match the number of emitted
                                documents. Each cursor returned from the <em>parallelCollectionScan</em> method is
                                functionally equivalent to the cursors returned from the the <em>find</em> method.</p>

                            <h1 id="toc_4">GridStore the Read/Write Stream</h1>

                            <p>Until now all the methods we have covered are <em>Readable</em> meaning they can only
                                provide a readable stream. GridStore implements the <em>Duplex</em> stream meaning it
                                can not only be read as a Stream (say stream a mp3 straight from your GridFS
                                collections) but also be written to (say upload a file directly via http into GridFS).
                                Let&rsquo;s look at the simple example of streaming a GridStore file and then one where
                                we use an incoming stream to write to GridFS.</p>

                            <h2 id="toc_5">Streaming a GridFS file to disk</h2>

                            <p>Streaming a GridStore file to disk is fairly simple. The example below reads in a pdf
                                file and saves it in GridFS. It then creates a GridStore instance pointing to the newly
                                saved pdf file and passes the stream to a file write stream using pipe.</p>
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    var pipeResult = gs.stream().pipe(fileStream);
                            ...
                            <p>In the case of writing a file to GridFS using streams we do the reverse piping the file
                                read stream into a our gridstore instance.</p>
...
<pre><code class="language-js">var MongoClient = require('mongodb').MongoClient
    ...
    fileStream.pipe(stream);
                ...
                <div class="col-md-1">
                    ...
                    <a class="navigation next" href="/tutorials/gridfs">
